...
import { useRoqClient, useEventFindFirst } from 'lib/roq';
...
function EventEditPage() {
  ...
  const { data, error, isLoading, mutate } = useEventFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: EventInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.event.update({
        data: values as RoqTypes.event,
        where: {
          id,
        },
      });
      ...
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <AsyncSelect<VendorInterface>
            ...
            fetcher={() => roqClient.vendor.findManyWithCount({})}
            labelField={'name'}
          />
