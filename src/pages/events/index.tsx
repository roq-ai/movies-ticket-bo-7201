...
import { useRoqClient, useEventFindManyWithCount } from 'lib/roq';
...
export function EventListPage(props: EventListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useEventFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.event.delete({ where: { id } });
      await mutate();
    } catch (error) {
